...
export default function EnrollModal({ isOpen, onClose, defaultCourse = '' }: Props) {
  ...
  useEffect(() => {
    if (isOpen) {
      setForm((f) => ({ ...f, course: defaultCourse }));
      setStatus('idle');
    ...
    try {
      const res = await fetch('https://api.web3forms.com/submit', {
        ...
        body: JSON.stringify({
          ...
          subject: `New Course Enquiry — ${form.course || 'General'}`,
          from_name: 'Akaris.ai Enquiry Form',
          name: form.name,
          email: form.email,
          phone: form.phone,
      ...
      <div className="relative bg-navy-900 border border-navy-800 rounded-2xl w-full max-w-md shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-navy-800">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-0.5">We'll be in touch with next steps.</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-6">
          {status === 'success' ? (
            <div className="text-center py-6">
              ...
              <h3 className="text-white font-bold text-lg mb-2">Enquiry Sent!</h3>
              <p className="text-slate-400 text-sm mb-6">
                Thanks, <span className="text-white">{form.name}</span>. We've received your enquiry
                and will reach out to <span className="text-white">{form.email}</span> shortly.
              </p>
              <button
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                >
                  <option value="" className="text-slate-500">Select a course (optional)</option>
                  {COURSES.map((c) => (
